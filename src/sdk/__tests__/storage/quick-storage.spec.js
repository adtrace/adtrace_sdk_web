...
describe('test low-level localStorage manipulation', () => {
  ...
  it('returns empty result when no data in localStorage', () => {
    ...
    expect(localStorage.getItem.mock.calls[0][0]).toEqual(`adtrace-sdk.${storeNames.queue.name}`)
    expect(localStorage.getItem.mock.calls[1][0]).toEqual(`adtrace-sdk.${storeNames.activityState.name}`)
    expect(localStorage.getItem.mock.calls[2][0]).toEqual(`adtrace-sdk.${storeNames.globalParams.name}`)
    expect(localStorage.getItem.mock.calls[3][0]).toEqual(`adtrace-sdk.${storeNames.preferences.name}`)
