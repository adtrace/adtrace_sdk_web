...
function expectNotRunningTrackEvent () {
  ...
  expect(Logger.default.log).toHaveBeenLastCalledWith('Adtrace SDK is disabled, can not track event')
...
function expectNotRunningTrackEventWhenNoInstance () {
  ...
  expect(Logger.default.error).toHaveBeenLastCalledWith('Adtrace SDK can not track event, sdk instance is not initialized')
  expect(GlobalParams.get).not.toHaveBeenCalled()
...
function expectNotRunningTrackEventWhenNoStorage () {
  ...
  expect(Logger.default.log).toHaveBeenLastCalledWith('Adtrace SDK can not track event, no storage available')
  expect(GlobalParams.get).not.toHaveBeenCalled()
...
function expectNotRunningStatic () {
  ...
  expect(Logger.default.log).toHaveBeenLastCalledWith('Adtrace SDK is disabled, can not add global callback parameters')
  expect(GlobalParams.add).not.toHaveBeenCalled()

  _instance.addGlobalPartnerParameters([{key: 'key', value: 'value'}])

  expect(Logger.default.log).toHaveBeenLastCalledWith('Adtrace SDK is disabled, can not add global partner parameters')
  expect(GlobalParams.add).not.toHaveBeenCalled()

  _instance.removeGlobalCallbackParameter('key')

  expect(Logger.default.log).toHaveBeenLastCalledWith('Adtrace SDK is disabled, can not remove global callback parameter')
  expect(GlobalParams.remove).not.toHaveBeenCalled()

  _instance.removeGlobalPartnerParameter('key')

  expect(Logger.default.log).toHaveBeenLastCalledWith('Adtrace SDK is disabled, can not remove global partner parameter')
  expect(GlobalParams.remove).not.toHaveBeenCalled()

  _instance.clearGlobalCallbackParameters()

  expect(Logger.default.log).toHaveBeenLastCalledWith('Adtrace SDK is disabled, can not remove all global callback parameters')
  expect(GlobalParams.removeAll).not.toHaveBeenCalled()

  _instance.clearGlobalPartnerParameters()

  expect(Logger.default.log).toHaveBeenLastCalledWith('Adtrace SDK is disabled, can not remove all global partner parameters')
  expect(GlobalParams.removeAll).not.toHaveBeenCalled()

  _instance.switchToOfflineMode()

  expect(Logger.default.log).toHaveBeenLastCalledWith('Adtrace SDK is disabled, can not set offline mode')
...
function expectNotRunningStaticWhenNoStorage () {
  ...
  expect(Logger.default.log).toHaveBeenLastCalledWith('Adtrace SDK can not add global callback parameters, no storage available')
  expect(GlobalParams.add).not.toHaveBeenCalled()

  _instance.addGlobalPartnerParameters([{key: 'key', value: 'value'}])

  expect(Logger.default.log).toHaveBeenLastCalledWith('Adtrace SDK can not add global partner parameters, no storage available')
  expect(GlobalParams.add).not.toHaveBeenCalled()

  _instance.removeGlobalCallbackParameter('key')

  expect(Logger.default.log).toHaveBeenLastCalledWith('Adtrace SDK can not remove global callback parameter, no storage available')
  expect(GlobalParams.remove).not.toHaveBeenCalled()

  _instance.removeGlobalPartnerParameter('key')

  expect(Logger.default.log).toHaveBeenLastCalledWith('Adtrace SDK can not remove global partner parameter, no storage available')
  expect(GlobalParams.remove).not.toHaveBeenCalled()

  _instance.clearGlobalCallbackParameters()

  expect(Logger.default.log).toHaveBeenLastCalledWith('Adtrace SDK can not remove all global callback parameters, no storage available')
  expect(GlobalParams.removeAll).not.toHaveBeenCalled()

  _instance.clearGlobalPartnerParameters()

  expect(Logger.default.log).toHaveBeenLastCalledWith('Adtrace SDK can not remove all global partner parameters, no storage available')
  expect(GlobalParams.removeAll).not.toHaveBeenCalled()

  _instance.switchToOfflineMode()

  expect(Logger.default.log).toHaveBeenLastCalledWith('Adtrace SDK can not set offline mode, no storage available')
  expect(Queue.setOffline).not.toHaveBeenCalled()
...
function _expectDestroy () {
  ...
  expect(Logger.default.log).toHaveBeenCalledWith('Adtrace SDK instance has been destroyed')
}

function _expectNotDestroy () {
  expectNotShutDown()
  expect(GdprForgetDevice.destroy).not.toHaveBeenCalled()
  expect(Logger.default.log).not.toHaveBeenCalledWith('Adtrace SDK instance has been destroyed')
}
