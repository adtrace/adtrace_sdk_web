...
describe('main entry point - test GDPR-Forget-Me when in initially enabled state', () => {
  ...
  describe('sdk: forget -> init -> flush', () => {
    ...
    it('does not run forget-me request yet', () => {
      ...
      suite.expectNotGdprRequest('Adtrace SDK will run GDPR Forget Me request after initialisation')
    })

    it('fails again to run forget-me request', () => {
      AdtraceInstance.gdprForgetMe()

      suite.expectNotGdprRequest('Adtrace SDK is already prepared to send GDPR Forget Me request')
    })
